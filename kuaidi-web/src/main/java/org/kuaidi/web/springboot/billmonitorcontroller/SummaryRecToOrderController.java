...
@RestController
@RequestMapping("/web/summaryRecToOrderController/")
public class SummaryRecToOrderController {
    ...
    public ResultVo getOrderShow(HttpServletRequest request ,String startTime , String endTime){
    	try {
	   		 ...
	   		 List<Map<String, Object>> list = rectoOrderService.getRecToByRegion(incment.getNumber() , startTime, endTime);
	   		 if(list != null) {
    ...
    public void outExcelOrderShow(HttpServletResponse response, String incId,
            ...
            List<Map<String, Object>> list = rectoOrderService.getRecToByRegion(incNum , startTime, endTime);
	   		if(list != null && list.size() > 0 ) {
	   			for(int i = 0 ; i < list.size() ; i++) {
	   				...
	   				if(rectoOrderItem != null ) {
                        ...
                        BigDecimal  qjs = (BigDecimal)rectoOrderItem.get("qjs");
                        String qjsStr = "0"; 
            			if(qjs != null ) {
            				qjsStr = qjs.intValue() + "";
            			}
            			text = new HSSFRichTextString(qjsStr);
                        cell.setCellValue(text);
