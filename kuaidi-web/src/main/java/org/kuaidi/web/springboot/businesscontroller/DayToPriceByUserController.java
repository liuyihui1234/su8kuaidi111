...
@RestController
@RequestMapping("/web/dayToPriceByUser/")
public class DayToPriceByUserController {
     ...
     public ResultVo getOrderShow(HttpServletRequest request ,String startTime , String endTime){
    	 try {
    		 ...
    		 List<Map<String, Object>> list = orderService.getDayToPriceByUser(incment.getNumber(), startTime, endTime);
     ...
     public void outExcelOrderShow(HttpServletResponse response, String incId,
    	 ...
    	 try {
    		 ...
    		 List<Map<String, Object>> list = orderService.getDayToPriceByUser(incNum, startTime, endTime);
    		 String[] header = {"业务员编号", "业务员名字","寄方付","收方付","月结",  "代收货款"};
             //声明一个工作簿
             HSSFWorkbook workbook = new HSSFWorkbook();
             //生成一个表格，设置表格名称为"学生表"
             HSSFSheet sheet = workbook.createSheet("每日营业统计(按业务员)");
             //设置表格列宽度为10个字节
            	 ...
            	 for(int i = 0 ; i < list.size() ; i++) {
            		 ...
            		 if(rectoOrderItem != null ) {
            			 ...
            			 HSSFCell cell = row.createCell(0);
              			 String  userid = (String)rectoOrderItem.get("userid");
               			 if(userid == null ) {
               				userid = "";
               			 }
               			 HSSFRichTextString text = new HSSFRichTextString(userid);
                         cell.setCellValue(text);
             ...
             //这后面可以设置导出Excel的名称，
             String fileName = "dayToPriceByUser" +  TimeDayUtil.getCurrentDate() + ".xls";
