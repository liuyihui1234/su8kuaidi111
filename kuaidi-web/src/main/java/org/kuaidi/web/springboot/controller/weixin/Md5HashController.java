...
@RestController
@RequestMapping("/test/md5Hash/")
public class Md5HashController {
	
	@RequestMapping("getMd5Hash")
    @CrossOrigin
    public ResultVo getMd5Hash(String userName , String password) {
		try {
			String rst = enPwd(userName, password);
			return ResultUtil.exec(true, "success", rst);
		}catch(Exception e ) {
			e.printStackTrace();
			return ResultUtil.exec(false, "fail", null);
		}
	}
	
	public String enPwd(String username, String pwd)
    {
		StringBuilder sb = new StringBuilder();
		sb.append(username).append(pwd);
		System.out.println(sb);
        return (new Md5Hash(sb.toString(), username, 2)).toHex().toString();
    }
	...
	public String enPwd1(String username, String pwd)
    {
        ...
        return (new Md5Hash((new StringBuilder()).append(username).append(pwd).toString(), username, 2)).toHex().toString();
    }
